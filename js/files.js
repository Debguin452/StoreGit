...
export function renderFiles() {
  ...
  // Flatten files across repos; each file carries a .dir field (default 'root')
  const allFiles = state.repoFiles.flatMap(g =>
    (g.files || []).map(f => ({ ...f, _repoIdx: g.repoIdx }))
  );
  // Folders are a simple name list per repo — merge and dedupe
  const allFolders = [...new Set(state.repoFiles.flatMap(g => g.folders || []))].sort();

  if (!allFiles.length && !allFolders.length) {
  ...
  if (currentFolder) {
    const inFolder = allFiles.filter(f => (f.dir || 'root') === currentFolder);
    if (!inFolder.length) {
      el.innerHTML = `<div class="empty-state"><p>Folder "${currentFolder}" is empty.<br>Upload a file to add it here.</p></div>`;
      return;
    }
    inFolder.sort((a, b) => {
    ...
    inFolder.forEach(f => el.appendChild(buildFileRow(f)));
    return;
  }

  // Root view: folders first, then root-level files
  for (const name of allFolders) {
    const count = allFiles.filter(f => (f.dir || 'root') === name).length;
    el.appendChild(buildFolderRow(name, count));
  }

  const rootFiles = allFiles.filter(f => (f.dir || 'root') === 'root');
  rootFiles.sort((a, b) => {
    const at = a.uploadedAt || a.name, bt = b.uploadedAt || b.name;
    return at < bt ? 1 : at > bt ? -1 : 0;
  });
  rootFiles.forEach(f => el.appendChild(buildFileRow(f)));
}
...
export function moveFileToFolderUI(f) {
  const allFolders = [...new Set(state.repoFiles.flatMap(g => g.folders || []))].sort();
  const currentDir = f.dir || 'root';
  const chips = ['root', ...allFolders]
  ...
  showModalHtml(
    ...
    `<p class="modal-field-label">Choose destination folder</p>
     ...
     <div style="display:flex;gap:8px;align-items:center;margin-top:10px">
       <input id="move-folder-dest" class="modal-input" placeholder="Or type a new folder name…" autocomplete="off" style="flex:1">
       <button type="button" class="btn btn-ghost btn-sm" id="move-mkdir-btn">+ New</button>
     </div>`,
    'Move', 'btn-primary',
    async () => {
      const dest = document.getElementById('move-folder-dest')?.value?.trim() || 'root';
      try {
