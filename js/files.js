...
export function renderFiles() {
  ...
  // System directories generated by chunked uploads — never shown to users
  const HIDDEN = new Set(['.chunks', '.manifests', '.sgkeys']);

  // Flatten all repo files and apply filter
  const all = state.repoFiles.flatMap(g =>
    (g.files || []).filter(f => {
      const n = (f.name || '');
      if (!n) return false;

      // Explicit folder entry from server: show only user-created (no dot prefix)
      if (f.isFolder) {
        const clean = n.replace(/\/+$/, '');
        return clean.length > 0 && !clean.startsWith('.') && !HIDDEN.has(clean);
      }

      // Skip files in hidden system directories
      const segs = n.split('/');
      return !segs.some(s => s.startsWith('.') || HIDDEN.has(s));
  ...
  // Inside a folder — show only files with that prefix
  if (currentFolder) {
    const prefix = currentFolder + '/';
    const inFolder = all.filter(f => !f.isFolder && (f.originalName || f.name).startsWith(prefix));
    if (!inFolder.length) {
  ...
  // Root view — group into folders then files
  const folderMap  = new Map();   // folderName → file[]
  const rootFiles  = [];

  for (const f of all) {
    if (f.isFolder) {
      // Server-provided explicit folder entry (from .storegit/.gitkeep marker)
      const name = (f.name || '').replace(/\/+$/, '');
      if (name && !HIDDEN.has(name) && !folderMap.has(name)) folderMap.set(name, []);
      continue;
    }
    const displayName = f.originalName || f.name;
    ...
    if (parts.length > 1) {
      const dir = parts[0];
      if (HIDDEN.has(dir) || dir.startsWith('.')) continue;  // skip system subdirs
  ...
  for (const [name, files] of [...folderMap.entries()].sort()) {
    el.appendChild(buildFolderRow(name, files.length));
  }

  // Then root files (sorted by date desc)
  rootFiles.sort((a, b) => {
    const at = a.uploadedAt || a.name, bt = b.uploadedAt || b.name;
    return at < bt ? 1 : at > bt ? -1 : 0;
  });
